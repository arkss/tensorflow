...
기존에 데이터를 500개로 만드는 작업에서 순차적 -> 랜덤하게 데이터 생성
"""
...
""" 데이터 불러오기 """
all_data = pd.read_csv("./data_16_label.csv" )
...
count_list = [0 for i in range(17)]
...
for label in range(1,17):
    ...
    print(new_index, copy_index)
    temp_new_index = new_index
    temp_copy_index = randint(copy_index,new_index-1)
    
    print("진행중입니다. 현재 sub_data의 크기는", len(sub_data))
    while len(sub_data) != 1000:
        if len(sub_data) % 100 == 0:
            print("진행중입니다. 현재 sub_data의 크기는", len(sub_data))
...
all_data_list = all_data_500.values.tolist()


# count_list = [0 for i in range(1,24)]
count_list = [0 for i in range(1,17)]
...
answer_matrix = [[0]*16 for i in range(16)]
...
plt.savefig("./acc_label16_alldata1000.png")
...
plt.savefig("./loss_label16_alldata1000.png")
